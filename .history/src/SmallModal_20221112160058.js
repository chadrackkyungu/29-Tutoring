    ...
    return (
        <Modal show={open} onHide={onClose} size="sm">
            ...
            <Modal.Footer className="d-flex justify-content-end p-2">
                <button className="btn btn-primary text-white w-25" onClick={onClose}> {cancel} </button>
                {
                    !deleteBtn ? null : (
                        <button className="btn btn-primary text-white" onClick={onClose}> {deleteBtn} </button>
                    )
                }
            </Modal.Footer>
        </Modal>
